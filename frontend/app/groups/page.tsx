...
interface Group {
  ...
  status: string;
  created_at: string;
  // Enriched fields from includeGroupInfo=true
...
export default function GroupsPage() {
  ...
  const [sortBy, setSortBy] = useState<
    "profit_loss" | "wallet" | "created_date"
  >("profit_loss");
  ...
  const fetchGroups = async () => {
    try {
      setLoading(true);
      // Fetch groups with Telegram info and holdings included
      ...
      if (data.success) {
        // Data is already enriched with all the information we need
        setGroups(data.data);
      } else {
        ...
        <div className="relative z-10 container mx-auto px-4 py-8 pt-24 md:pt-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <Card
                key={i}
                className="border-purple-500/20 backdrop-blur-md bg-gradient-to-br from-purple-950/10 to-transparent"
              >
                <CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-purple-500/20 hover:bg-purple-500/5">
                    ...
                    <TableHead className="text-purple-300 font-semibold text-center">
                      Action
                    </TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {groups
                    .sort((a, b) => {
                      if (sortBy === "profit_loss") {
                        const aProfit =
                          a.profit_loss !== undefined
                            ? Number(a.profit_loss)
                            : -Infinity;
                        const bProfit =
                          b.profit_loss !== undefined
                            ? Number(b.profit_loss)
                            : -Infinity;
                        return bProfit - aProfit;
                      } else if (sortBy === "wallet") {
                        const aWallet =
                          a.wallet_balance_sol !== undefined
                            ? Number(a.wallet_balance_sol)
                            : -Infinity;
                        const bWallet =
                          b.wallet_balance_sol !== undefined
                            ? Number(b.wallet_balance_sol)
                            : -Infinity;
                    ...
                    .map((group, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell>
                            <div className="flex flex-col">
                              <span className="text-foreground font-semibold">
                                {group.telegram?.title ||
                                  `Group ${group.tgid.slice(-6)}`}
                              </span>
                          ...
                          <TableCell className="font-mono">
                            {group.statistics?.total_sol_invested !==
                            undefined ? (
                              <span className="text-foreground font-semibold">
                                {Number(
                                  group.statistics.total_sol_invested
                          ...
                          <TableCell>
                            {group.statistics?.total_active_orders !==
                              undefined &&
                            group.statistics?.total_sold_orders !==
                              undefined ? (
                              <div className="flex items-center gap-2">
                                <Badge className="bg-green-500/20 text-green-400 border-green-500/30">
                                  {group.statistics.total_active_orders} active
                                </Badge>
                                <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/30">
                                  {group.statistics.total_sold_orders} sold
                                </Badge>
                              </div>
                            ) : (
                              <span className="text-foreground/40 text-sm">
                          ...
                          <TableCell>
                            {group.token_holdings &&
                            group.token_holdings.length > 0 ? (
                              <div className="flex flex-wrap gap-1">
                                ...
                                {group.token_holdings.length > 3 && (
                                  <span className="text-xs text-foreground/60">
                                    +{group.token_holdings.length - 3} more
                                  </span>
                                )}
                              </div>
                            ) : (
